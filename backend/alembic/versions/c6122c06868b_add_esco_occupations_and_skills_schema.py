...
def downgrade() -> None:
    ...
    if bind.dialect.name == "sqlite":
        with op.batch_alter_table("skills") as batch_op:
            batch_op.drop_index(batch_op.f('ix_skills_title'))
            batch_op.drop_index(batch_op.f('ix_skills_esco_uri'))
            batch_op.alter_column('name', existing_type=sa.String(length=255), type_=sa.VARCHAR(length=100), existing_nullable=False)
